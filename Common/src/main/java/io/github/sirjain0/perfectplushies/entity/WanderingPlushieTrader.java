...
public class WanderingPlushieTrader extends WanderingTrader implements GeoEntity {
    ...
    private Int2ObjectMap<VillagerTrades.ItemListing[]> getTrades() {
        return toIntMap(ImmutableMap.of(
                1, BlockInit.plushieBlocks.stream().map(blockRegistryObject -> new VillagerTrades.ItemsForEmeralds(blockRegistryObject.get(), 5, 1, 1, 1)).toArray(VillagerTrades.ItemListing[]::new),
    ...
    protected void updateTrades() {
        Int2ObjectMap<VillagerTrades.ItemListing[]> trades = getTrades();
